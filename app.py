...
st.markdown("""
This analysis covers 120 years of Olympic data, up to the 2016 Olympics. 
Please note that the dataset may contain a few inconsistencies or missing values.
""")

df = pd.read_csv('csvs/athlete_events.csv')
region_df = pd.read_csv('csvs/noc_regions.csv')
...
if user_menu == ('Athelete wise Analysis'):
    ...
    fig = ff.create_distplot(x,name,show_hist=False,show_rug=False)# We can use above code to also implement Silver and Bronze Medalists
    fig.update_layout(autosize=False,width=1000,height=600)
